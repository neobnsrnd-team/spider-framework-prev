...
기존 금융 시스템에서 새로운 서비스를 추가하려면 AP 서버마다 TCP 통신 코드, 배치 실행 코드, 이력 관리 코드를 반복 작성해야 했다. Spider-Framework는 이 공통 관심사를 `spider-common`, `spider-link`, `spider-batch` 라이브러리로 분리하여, AP 서버는 **비즈니스 로직만** 구현하면 되는 구조를 제공한다.

**FWK 메타 기반 동적 라우팅**이 핵심이다. 커맨드 라우팅 규칙을 DB 메타테이블(`FWK_SERVICE`, `FWK_COMPONENT` 등)에 정의하면, 코드 변경·재배포 없이 `spider-admin`에서 reload API 호출만으로 서비스를 추가하거나 수정할 수 있다.

### 무엇이 구현되었나

| 구성요소                  | 구현 내용                                                                                                                                    |
|-----------------------|------------------------------------------------------------------------------------------------------------------------------------------|
| `spider-common`       | 공통 인프라 라이브러리 — TCP 프로토콜 모델(`JsonCommandRequest/Response`, `ManagementContext`), 관리 명령 처리, 로그레벨 조정. spider-link·spider-batch·spider-admin이 공통 참조 |
| `spider-link`         | TCP 연계엔진 라이브러리 — AP 서버에 내장되어 JSON / 고정길이 / ObjectStream 프로토콜의 TCP 서버를 제공 <br/> 메타 기반 라우팅 — FWK 메타테이블 체인으로 커맨드를 동적 실행. 재기동 없이 커맨드 추가·수정 가능 |
| `spider-batch`        | 배치 오케스트레이션 라이브러리 — `JobProvider` SPI 구현만으로 배치 실행·이력·TCP 연동·REST API가 자동 제공됨                                                              |
| `spider-admin`        | 관리 콘솔 — FWK 메타·메뉴·권한·사용자·긴급공지 등 플랫폼 전체를 웹 UI로 관리                                                                                         |
| `reactPlatform`       | React 통합 개발 플랫폼 — 리액트 코드 생성 & 결재 관리 서버                                                                                         |
| `reactive-springware` | 프론트엔드 컴포넌트 — 금융 앱 특화 React 컴포넌트 라이브러리 + 디자인 토큰 시스템. 프론트엔드 프로젝트가 path alias로 직접 참조                                   |
| `demo`                | `demo/bizApp` + `demo/front` — spider-link 내장형 금융 AP 서버군(인증·이체·채널·계정계 Mock)과 카드 서비스 프론트엔드 (시연용)                                          |
...
│                                                                          │
│  ┌─────────────────────────────────────────────────────────────────────┐ │
│  │ spider-common — TCP 모델·관리 명령·로그레벨 (공통 라이브러리)            │ │
│  └──────────────────────────┬──────────────────────────────────────────┘ │
│                             │ Maven 의존                                  │
│         ┌───────────────────┼───────────────────────┐                    │
│         ↓                   ↓                       ↓                    │
│  ┌──────────────┐   ┌───────────────────┐   ┌──────────────┐             │
│  │ spider-link  │   │ spider-batch      │   │ spider-admin │             │
│  │ TCP 연계엔진  │   │ 배치 오케스트레이션  │   │  관리 콘솔    │             │
│  │  (라이브러리) │   │  (라이브러리)       │   │              │             │
│  └──────┬───────┘   └──────┬────────────┘   └──────────────┘             │
│         │ Maven 내장        │ Maven 내장                                  │
└─────────┼──────────────────┼─────────────────────────────────────────────┘
          ↓                  ↓
   demo/AP 서버,          batch-was
   spider-batch,
   spider-admin
```

- **spider-common** — TCP 프로토콜 모델과 관리 명령 처리를 담당하는 공유 라이브러리. spider-link·spider-batch·spider-admin이 모두 참조한다.
- **spider-link** — AP 서버, spider-batch, spider-admin에 Maven 의존성으로 내장. `SpiderTcpServer` Bean만 등록하면 표준 TCP 서버를 갖춘다.
...
- **spider-admin** — FWK 메타테이블을 관리하는 운영 콘솔. 메타 변경 후 reload API를 호출하면 AP 서버 재기동 없이 라우팅이 갱신된다.
...
Spider-Framework/
│
├── spider-admin/            관리자 콘솔 (Spring Boot + Thymeleaf)
│   └── domain/              61개 비즈니스 모듈 (batch, gateway, monitor, user 등)
│
├── spider-common/           공통 TCP 모델·관리 명령·로그레벨 조정 라이브러리
│
├── spider-link/             TCP 연계엔진 라이브러리 — AP 서버·spider-batch·spider-admin에 직접 내장
│
...
| 프로젝트 | 역할 |
|----------|------|
| `spider-admin` | 플랫폼 전체 관리 콘솔 (FWK 메타·메뉴·권한·긴급공지 등) |
| `spider-common` | TCP 모델·관리 명령·로그레벨 조정 공통 **라이브러리** — spider-link·spider-batch·spider-admin에 공유 |
...
| `demo/bizApp` | 시연용 금융 AP 서버군 (채널/인증/이체/계정계 Mock) |
| `demo/front` | 시연용 프론트엔드 (React, `@cl`로 component-library 참조) |
...
| 모듈 | README |
|------|--------|
| spider-admin | [spider-admin/README.md](spider-admin/README.md) |
| spider-common | [spider-common/README.md](spider-common/README.md) |
