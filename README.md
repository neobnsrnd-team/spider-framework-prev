...
| 구성요소                  | 구현 내용                                                                                                                                    |
|-----------------------|------------------------------------------------------------------------------------------------------------------------------------------|
| `spider-common`       | 공통 인프라 라이브러리 — TCP 프로토콜 모델(`JsonCommandRequest/Response`, `ManagementContext`), 관리 명령 처리, 로그레벨 조정. spider-link·spider-batch·admin이 공통 참조 |
...
| `admin`               | 관리 콘솔 — FWK 메타·메뉴·권한·사용자·긴급공지 등 플랫폼 전체를 웹 UI로 관리                                                                                         |
...
- **spider-common** — TCP 프로토콜 모델과 관리 명령 처리를 담당하는 공유 라이브러리. spider-link·spider-batch·admin이 모두 참조한다.
...
│
├── spider-link/             TCP 연계엔진 라이브러리 — AP 서버·spider-batch·admin에 직접 내장
│
...
| `spider-common` | TCP 모델·관리 명령·로그레벨 조정 공통 **라이브러리** — spider-link·spider-batch·admin에 공유 |
...
| 순서 | 모듈 | 명령 | 이유 |
|------|------|------|------|
| 1 | `spider-common` | `mvn install -f spider-common/pom.xml` | spider-link·spider-batch·admin이 공통 의존 |
| 2 | `spider-link` | `mvn install -f spider-link/pom.xml` | spider-batch·admin·bizApp 서버가 의존 |
