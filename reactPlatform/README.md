...
- [Figma → React 속성 매핑 (VariantNormalizer)](#figma--react-속성-매핑-variantnormalizer)
...
## Figma → React 속성 매핑 (VariantNormalizer)

Figma 컴포넌트 속성(`componentProperties`)의 키와 값은 Figma 규칙에 따라 PascalCase로 저장된다.
React 컴포넌트의 prop은 camelCase 키와 소문자 또는 정해진 값으로 구성된다.
`VariantNormalizer`는 이 간극을 메운다.
...
FigmaDesignContext → PromptBuilder → Claude API
```

### variant-mapping.json 구조

`src/main/resources/variant-mapping.json`이 매핑 규칙을 담는다.

| 섹션 | 역할 | 예시 |
|------|------|------|
| `global.keys` | PascalCase 키 → camelCase 키 (전역 적용) | `"Disabled" → "disabled"` |
| `components.{이름}.keys` | 특정 컴포넌트 전용 키 오버라이드 | `input`의 `"ValidationState" → "validationState"` |
| `values.{키}` | 값 변환 테이블 (키별 명시적 매핑) | `"Large" → "lg"` |
| `values.default` | `"lowercase"` — 테이블에 없는 값의 기본 전략 | `"True" → "true"` |

> **주의**: `normalizeKey()`는 `global.keys`에 없는 키를 **그대로 통과**시킨다 (소문자 변환 없음).  
> figma-plugin에서 새 속성 키를 추가하면 반드시 `global.keys`에도 등록해야 한다.

### variant-mapping.json 업데이트 시점

figma-plugin의 `create*.ts`에서 새 속성(variant 이름)을 사용할 때:

1. 해당 키가 `global.keys`에 없으면 추가한다
   - 예: `"IsSelected": "isSelected"`, `"Cols": "cols"`
2. 값이 소문자 변환 외의 변환이 필요하면 `values.{키}` 테이블에 추가한다
   - 예: `size` — `"Small"/"Medium"/"Large"` → `"sm"/"md"/"lg"`
   - 예: camelCase 값을 그대로 유지해야 하는 경우 → 동일값 매핑으로 보호 (`"foreignDeposit": "foreignDeposit"`)
