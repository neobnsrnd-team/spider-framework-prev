...
| 메뉴 이름 | 커맨드 | 담당 파일 | 설명 |
|---|---|---|---|
| 컴포넌트 생성 | `createComponents` | `commands/createComponents.ts` | 전체 컴포넌트를 섹션별로 캔버스에 생성 |
| 아이콘 생성 (Instance Swap용) | `createIcons` | `commands/createIcons.ts` | `Icons/*` ComponentNode 일괄 생성 |
| 변수 등록 (Primitives + Semantic) | `createVariables` | `commands/createVariables.ts` | Figma 색상 변수 등록 |

### 아이콘 생성 커맨드 상세

"아이콘 생성" 커맨드는 `utils/icons.ts`의 lucide-react 아이콘 데이터를 기반으로  
...
`utils/icons.ts`는 `npm run generate:icons`로 자동 생성된다. lucide-react 버전 업 시 재실행한다.
...
import { COLOR, SPACING, FONT_SIZE } from '../../utils/tokens';
import { createComponent, setAutoLayout, setPadding, setFill, addTextWithVar } from '../../utils/helpers';
...
> `../../utils/` 경로는 `components/core/`, `components/layout/` 기준이다.  
> `components/biz/**`, `components/modules/**` 하위 파일은 `../../../utils/`를 사용한다.

**Variant가 있는 경우:**

```ts
import { combineVariants } from '../../utils/helpers';
...
```ts
import { addIconSlot } from '../../utils/helpers';
...
// commands/createComponents.ts 기준 경로
...
2. `commands/createComponents.ts`에서 import 제거
3. `commands/createComponents.ts`의 배열에서 `await s('create[ComponentName]', create[ComponentName])` 제거
4. `commands/createComponents.ts` 상단 JSDoc 주석의 컴포넌트 목록에서 제거

---

## 8. utils/helpers.ts 주요 유틸 함수
...
> **utils/tokens.ts는 직접 수정하지 않는다.**  
> 새 토큰이 필요하다고 판단되더라도 임의로 추가하지 않고 개발자에게 확인한다.  
> `utils/tokens.ts`는 `design-tokens/globals.css`와 동기화 절차를 통해서만 업데이트된다.
...
## 10. reactPlatform과의 연동 — VariantNormalizer

figma-plugin이 생성하는 Figma 컴포넌트의 속성 이름은 **reactPlatform의 코드 생성 품질에 직접 영향**을 준다.

### 흐름

```
figma-plugin
  create*.ts 에서 createComponent('Disabled=False, Size=Large') 작성
        ↓
Figma 캔버스에 컴포넌트 저장
        ↓
reactPlatform — FigmaDesignExtractor
  Figma API로 componentProperties 읽음: { "Disabled": "False", "Size": "Large" }
        ↓
VariantNormalizer (variant-mapping.json 참조)
  키 정규화: "Disabled" → "disabled", "Size" → "size"
  값 정규화: "False" → "false", "Large" → "lg"
        ↓
Claude API — { "disabled": "false", "size": "lg" }
```

### variant-mapping.json

매핑 데이터 파일: `reactPlatform/src/main/resources/variant-mapping.json`

| 섹션 | 역할 |
|------|------|
| `global.keys` | PascalCase 키 → camelCase 키 (전역) |
| `components.{이름}.keys` | 특정 컴포넌트 전용 키 오버라이드 |
| `values.{키}` | 값 변환 테이블 |
| `values.default` | `"lowercase"` — 테이블 미등록 값의 기본 전략 |

### 속성 키 작성 규칙

`create*.ts`에서 variant 속성 이름을 정할 때는 아래를 따른다.

1. **`global.keys`에 이미 등록된 이름을 우선 사용한다.**
   이미 등록된 키 목록은 `variant-mapping.json`의 `global.keys`를 확인한다.
   예: `Disabled`, `Size`, `Variant`, `State`, `Shape`, `Mode` 등

2. **새 속성 키를 추가했으면 반드시 `global.keys`에도 등록한다.**
   등록하지 않으면 `VariantNormalizer`가 PascalCase 그대로 Claude에 전달하여
   React prop 이름과 불일치가 생긴다.

3. **값이 소문자 변환 외의 변환이 필요하면 `values.{키}` 테이블에 추가한다.**
   - 예: `Small/Medium/Large` → `sm/md/lg`
   - 예: camelCase 값을 그대로 유지해야 하는 경우 동일값 매핑으로 보호

> `VariantNormalizer`는 `global.keys`에 없는 키를 소문자 변환 없이 그대로 통과시킨다.

---

## 11. Claude로 작업하기

Claude에게 작업을 맡기면 파일 생성부터 `commands/createComponents.ts` 등록까지 한 번에 처리할 수 있다.
...
- commands/createComponents.ts에 import 및 moduleCommonNodes 배열 등록
```
